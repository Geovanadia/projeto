...
if (($result_usuario) AND ($result_usuario->rowCount() != 0)) {
    $query_del_usuario = "DELETE FROM carros WHERE id = $id";
    $apagar_usuario = $conn->prepare($query_del_usuario);

    if ($apagar_usuario->execute()) {
        $_SESSION['msg'] = "<p style='color: green;'>Carro apagado com sucesso!</p>";
        header("Location: consulta.php");
    } else {
        $_SESSION['msg'] = "<p style='color: #f00;'>Carro não apagado com sucesso!</p>";
        header("Location: consulta.php");
    }
} else {
